...
def _rule_based_alerts() -> list[dict]:
    ...
    if rx["over_90"] > 0:
        alerts.append({
            ...
            "description": f"Bs {rx['over_90']:,.0f} en facturas vencidas más de 90 días.",
    ...
    if rx["top5_concentration_pct"] > 50:
        alerts.append({
            "level": "warning",
            "module": "receivables",
    ...
    if px["critical_count"] > 0:
        alerts.append({
            ...
            "description": f"Bs {px['critical_amount']:,.0f} en pagos con prioridad crítica.",
    ...
    if px["due_next_7d"] > 0:
        alerts.append({
            "level": "warning",
            "module": "payables",
            "title": f"CxP próximas: Bs {px['due_next_7d']:,.0f} en 7 días",
    ...
    if ix["dead_stock_value"] > 0:
        alerts.append({
            "level": "info",
            "module": "inventory",
            "title": f"Dead stock: Bs {ix['dead_stock_value']:,.0f}",
    ...
    # Expenses
    ex = expense_kpis()
    if ex["variance_pct"] > 10:
        alerts.append({
            "level": "warning",
            "module": "expenses",
            "title": f"Gastos {ex['variance_pct']:+.1f}% sobre presupuesto",
            "description": f"Desviación de Bs {ex['total_variance']:,.0f} vs presupuesto.",
            "action": "Revisar partidas con mayor desviación.",
        })
    elif ex["variance_pct"] < -5:
        alerts.append({
            "level": "positive",
            "module": "expenses",
    ...
    # Cashflow
    cx = cashflow_kpis()
    if cx["runway_days"] is not None and cx["runway_days"] < 30:
        alerts.append({
            "level": "critical",
            "module": "cashflow",
...
def ai_enhanced_alerts(engine: Any) -> str:
    ...
    prompt = (
        ...
        "Contexto adicional:\n"
        f"Ventas del mes: Bs {summary['sales']['revenue']:,.0f}\n"
        f"Cash actual: Bs {summary['cashflow']['current_balance']:,.0f}\n\n"
